...
class DatabaseSeeder extends Seeder
{
    public function run(): void
    {
        ...
        */
        ...
        /*
        |--------------------------------------------------------------------------
        | 12. STOCK, FOURNISSEURS, CLIENTS ET FACTURATION
        |--------------------------------------------------------------------------
        |
        | Le cycle commercial utilisé par HOPE est :
        | PROFORMA -> FACTURE DE SOLDE / CLÔTURE -> AVOIR.
        | Aucun devis n'est créé ici.
        */

        $this->command?->info('Création des données commerciales...');

        $centralWarehouse = Warehouse::firstOrCreate(
            ['code' => 'HOPE-CENTRAL'],
            ['name' => 'Entrepôt central HOPE', 'address' => 'Bamako, Mali', 'phone' => '+22370000099', 'is_active' => true]
        );

        // Produits / kits vendables.
        $products = [];
        ...
        $productDefinitions = [
            [
                ...
                'stock_min' => 20,
                'currency' => 'XOF',
                'description' => 'Kit Néné complet destiné aux programmes de santé maternelle.',
            ],
            [
                'sku' => 'KIT-NENE-PLUS',
                'name' => 'Kit Néné Plus',
                ...
                'stock_min' => 10,
                'currency' => 'XOF',
                'description' => 'Version renforcée du Kit Néné.',
            ],
            [
                'sku' => 'KIT-HYGIENE',
                'name' => 'Kit Hygiène Maternelle',
                'unit' => 'kit',
                'purchase_price' => 5000,
                'sale_price' => 6500,
                'stock_quantity' => 60,
                'stock_min' => 15,
                'currency' => 'XOF',
                'description' => 'Kit complémentaire pour l\'hygiène maternelle.',
            ],
        ];

        foreach ($productDefinitions as $data) {
            $products[$data['sku']] = InventoryProduct::updateOrCreate(
                ['sku' => $data['sku']],
                [
                    'name' => $data['name'],
                    'unit' => $data['unit'],
        ...
        // Stock par entrepôt : la somme correspond au stock global de démonstration.
        foreach ($products as $product) {
            WarehouseStock::updateOrCreate(
                ['warehouse_id' => $centralWarehouse->id, 'inventory_product_id' => $product->id],
                ['quantity' => $product->stock_quantity, 'reserved_quantity' => 0]
            );
        }
        ...
        $supplierDefinitions = [
            [
                'name' => 'Fournitures Santé Mali',
                'contact_name' => 'Mamadou Konaté',
                'phone' => '+22376000101',
                'email' => 'contact@fournituressante.test',
                'address' => 'Bamako, Mali',
                'tax_number' => 'ML-TAX-0001',
            ],
            [
                'name' => 'Mali Medical Supply',
                'contact_name' => 'Awa Traoré',
                'phone' => '+22376000102',
                'email' => 'commercial@malimedical.test',
                'address' => 'Bamako, Mali',
                'tax_number' => 'ML-TAX-0002',
            ],
        ];

        $suppliers = [];

        foreach ($supplierDefinitions as $data) {
            $suppliers[$data['name']] = Supplier::updateOrCreate(
                ['name' => $data['name']],
                [
        ...
        // Entrées de stock de référence.
        foreach ($productDefinitions as $data) {
            $product = $products[$data['sku']];
            $reference = 'SEED-ENTRY-' . $data['sku'];
            ...
            StockMovement::updateOrCreate(
                ...
                [
                    ...
                    'supplier_id' => $suppliers['Fournitures Santé Mali']->id,
                    'user_id' => $direction->id,
                ]
            );
        }

        // Clients / organisations acheteuses.
        $customerDefinitions = [
            [
                'customer_code' => 'ORG-001',
                'name' => 'Association Santé Maternelle du Mali',
                'contact_name' => 'Aminata Coulibaly',
                'phone' => '+22377000101',
                'email' => 'contact@asmm.test',
                'address' => 'Bamako, Mali',
                'tax_number' => 'ML-CUST-0001',
            ],
            [
                'customer_code' => 'ORG-002',
                'name' => 'ONG Espoir Santé',
                'contact_name' => 'Moussa Diarra',
                'phone' => '+22377000102',
                'email' => 'achats@espoirsante.test',
                'address' => 'Sikasso, Mali',
                'tax_number' => 'ML-CUST-0002',
            ],
            [
                'customer_code' => 'ORG-003',
                'name' => 'Centre de Santé Communautaire de Kayes',
                'contact_name' => 'Dr. Fatoumata Diallo',
                'phone' => '+22377000103',
                'email' => 'direction@cscomkayes.test',
                'address' => 'Kayes, Mali',
                'tax_number' => 'ML-CUST-0003',
            ],
        ];

        $customers = [];

        foreach ($customerDefinitions as $data) {
            $customers[$data['customer_code']] = Customer::updateOrCreate(
        ...
        // Helper de calcul des lignes commerciales.
        $calculateLine = static function (int $quantity, float $unitPrice, float $discount = 0): float {
            return round(($quantity * $unitPrice) - $discount, 2);
        };

        // 1) Une proforma acceptée : elle représente la demande validée
        // par l'organisation avant sa transformation en facture.
        $proforma = CommercialDocument::updateOrCreate(
            ['document_number' => 'PRO-2026-0001'],
            [
                'type' => 'proforma',
                'customer_id' => $customers['ORG-001']->id,
                ...
                'payment_method' => null,
                'status' => 'accepted',
                'notes' => 'Proforma validée par l’organisation. Prête à être transformée en facture.',
                'created_by' => $direction->id,
                'stock_applied_at' => null,
            ]
        );

        CommercialDocumentLine::updateOrCreate(
            [
                'commercial_document_id' => $proforma->id,
                'inventory_product_id' => $products['KIT-NENE']->id,
            ],
            [
                'quantity' => 20,
                'unit_price' => $products['KIT-NENE']->sale_price,
                'discount' => 0,
                'line_total' => $calculateLine(20, (float) $products['KIT-NENE']->sale_price),
            ]
        );

        // 1 bis) Une proforma brouillon pour tester tout le workflow depuis zéro.
        $draftProforma = CommercialDocument::updateOrCreate(
            ['document_number' => 'PRO-2026-0002'],
            [
                'type' => 'proforma',
                'customer_id' => $customers['ORG-002']->id,
                'source_document_id' => null,
                'issue_date' => now()->toDateString(),
                'due_date' => now()->addDays(30)->toDateString(),
                'valid_until' => now()->addDays(15)->toDateString(),
                ...
                'payment_method' => null,
                'status' => 'draft',
                'notes' => 'Proforma brouillon de démonstration : envoyer puis accepter pour créer la facture.',
                'created_by' => $direction->id,
                'stock_applied_at' => null,
            ]
        );
        CommercialDocumentLine::updateOrCreate(
            ['commercial_document_id' => $draftProforma->id, 'inventory_product_id' => $products['KIT-HYGIENE']->id],
            ['quantity' => 10, 'unit_price' => $products['KIT-HYGIENE']->sale_price, 'discount' => 0, 'line_total' => 65000]
        );

        // 2) Une facture de solde / clôture issue de la proforma.
        // Elle est volontairement fermée afin de fournir un cas de test
        // immédiatement exploitable dans le module de facturation.
        $invoice = CommercialDocument::updateOrCreate(
            ...
            [
                ...
                'status' => 'closed',
                'notes' => 'Facture de solde / clôture générée à partir de la proforma PRO-2026-0001.',
                'created_by' => $direction->id,
                'stock_applied_at' => now()->subDays(5),
            ]
        );
        ...
        // Sortie de stock correspondant à la facture clôturée.
        StockMovement::updateOrCreate(
            ['reference' => 'FAC-2026-0001-STOCK'],
            [
                'inventory_product_id' => $products['KIT-NENE']->id,
                'type' => 'exit',
                'quantity' => 20,
                ...
                'stock_after' => 80,
                'unit_price' => $products['KIT-NENE']->sale_price,
                'reason' => 'Sortie liée à la facture FAC-2026-0001',
                'supplier_id' => null,
                'user_id' => $direction->id,
            ]
        );

        // 3) Un avoir de démonstration partiel : 2 kits retournés.
        // Cela permet de tester immédiatement le calcul du reste créditable.
        $creditNote = CommercialDocument::updateOrCreate(
            ['document_number' => 'AVO-2026-0001'],
            [
                'type' => 'credit_note',
                'customer_id' => $customers['ORG-001']->id,
                ...
                'payment_method' => null,
                'status' => 'closed',
                'notes' => 'Avoir automatique de démonstration lié à FAC-2026-0001 : retour de 2 kits.',
                'created_by' => $direction->id,
                'stock_applied_at' => now()->subDays(2),
            ]
        );

        CommercialDocumentLine::updateOrCreate(
            [
                'commercial_document_id' => $creditNote->id,
                'inventory_product_id' => $products['KIT-NENE']->id,
            ],
            [
                'quantity' => 2,
                'unit_price' => $products['KIT-NENE']->sale_price,
                'discount' => 0,
                'line_total' => $calculateLine(2, (float) $products['KIT-NENE']->sale_price),
            ]
        );

        StockMovement::updateOrCreate(
            ['reference' => 'AVO-2026-0001-STOCK'],
            [
                'inventory_product_id' => $products['KIT-NENE']->id,
                'type' => 'entry',
                'quantity' => 2,
                ...
                'stock_after' => 82,
                'unit_price' => $products['KIT-NENE']->sale_price,
                'reason' => 'Retour en stock lié à l’avoir AVO-2026-0001',
                'supplier_id' => null,
                'user_id' => $direction->id,
            ]
        );

        // Etat final cohérent avec les mouvements de démonstration : 100 - 20 + 2 = 82.
        $products['KIT-NENE']->update(['stock_quantity' => 82]);
        WarehouseStock::updateOrCreate(
            ['warehouse_id' => $centralWarehouse->id, 'inventory_product_id' => $products['KIT-NENE']->id],
            ['quantity' => 82, 'reserved_quantity' => 0]
        );

        // Un paiement réel de démonstration correspondant à la facture clôturée.
        Payment::updateOrCreate(
            ['commercial_document_id' => $invoice->id, 'reference' => 'PAY-SEED-FAC-2026-0001'],
            [
                'customer_id' => $invoice->customer_id,
                'recorded_by' => $direction->id,
                'amount' => 200000,
                'paid_at' => now()->subDays(5),
                'method' => 'bank_transfer',
                'status' => 'validated',
                'notes' => 'Paiement de démonstration correspondant à la facture clôturée.',
            ]
        );

        // Une facture ouverte immédiatement testable : acceptée et avec un solde.
        $openInvoice = CommercialDocument::updateOrCreate(
            ...
            [
                ...
                'payment_method' => null,
                'status' => 'sent',
                'notes' => 'Facture ouverte de démonstration : acceptez-la puis enregistrez un paiement partiel ou total.',
                'created_by' => $direction->id,
                'stock_applied_at' => null,
            ]
        );
        CommercialDocumentLine::updateOrCreate(
            ['commercial_document_id' => $openInvoice->id, 'inventory_product_id' => $products['KIT-HYGIENE']->id],
            [
                'quantity' => 10,
                'unit_price' => $products['KIT-HYGIENE']->sale_price,
                'discount' => 0,
                'line_total' => 65000,
            ]
        );
        CommercialDocumentLine::updateOrCreate(
            ['commercial_document_id' => $openInvoice->id, 'inventory_product_id' => $products['KIT-NENE-PLUS']->id],
            [
                'quantity' => 1,
                'unit_price' => $products['KIT-NENE-PLUS']->sale_price,
                'discount' => 0,
                'line_total' => 13500,
            ]
        );
        // Le total est recalé explicitement pour refléter les deux lignes.
        $openInvoice->update(['subtotal' => 78500, 'total_amount' => 78500, 'balance_amount' => 78500]);

        // Une commande client brouillon immédiatement testable dans l'interface.
        $demoOrder = SalesOrder::updateOrCreate(
            ...
            [
                ...
                'total_amount' => 40500,
                'currency' => 'XOF',
                'notes' => 'Commande de démonstration : valider puis créer une livraison.',
                'created_by' => $direction->id,
            ]
        );
        SalesOrderLine::updateOrCreate(
            ['sales_order_id' => $demoOrder->id, 'inventory_product_id' => $products['KIT-NENE-PLUS']->id],
            ['quantity' => 3, 'delivered_quantity' => 0, 'unit_price' => $products['KIT-NENE-PLUS']->sale_price, 'discount' => 0, 'line_total' => 40500]
        );

        // Une commande fournisseur brouillon immédiatement testable.
        $demoPurchase = PurchaseOrder::updateOrCreate(
            ['order_number' => 'BCF-2026-0001'],
            [
                'supplier_id' => $suppliers['Fournitures Santé Mali']->id,
                'warehouse_id' => $centralWarehouse->id,
                'status' => 'draft',
                ...
                'total_amount' => 150000,
                'currency' => 'XOF',
                'notes' => 'Commande fournisseur de démonstration.',
                'created_by' => $direction->id,
            ]
        );
        PurchaseOrderLine::updateOrCreate(
            ['purchase_order_id' => $demoPurchase->id, 'inventory_product_id' => $products['KIT-HYGIENE']->id],
            ['quantity' => 30, 'received_quantity' => 0, 'unit_price' => $products['KIT-HYGIENE']->purchase_price, 'discount' => 0, 'line_total' => 150000]
        );
        ...
        */
        ...
        $this->command?->info('Produits / kits vendables : ' . InventoryProduct::count());
        $this->command?->info('Fournisseurs : ' . Supplier::count());
        $this->command?->info('Clients / organisations : ' . Customer::count());
        $this->command?->info('Mouvements de stock : ' . StockMovement::count());
        $this->command?->info('Documents commerciaux : ' . CommercialDocument::count());
        $this->command?->info('  - Proformas : ' . CommercialDocument::where('type', 'proforma')->count());
        $this->command?->info('  - Factures : ' . CommercialDocument::where('type', 'invoice')->count());
        $this->command?->info('  - Avoirs : ' . CommercialDocument::where('type', 'credit_note')->count());
        ...
        $this->command?->info('==============================================');
        $permissionNames = ['dashboard.view','stock.view','stock.manage','products.manage','warehouses.manage','customers.view','customers.manage','orders.manage','documents.view','documents.manage','payments.view','payments.manage','reports.view','kits.manage','users.manage','roles.manage','settings.manage','purchases.view','purchases.manage'];
        foreach ($permissionNames as $name) Permission::firstOrCreate(['name'=>$name,'guard_name'=>'web']);
        $admin = Role::firstOrCreate(['name'=>'Administrateur','guard_name'=>'web']);
        $admin->syncPermissions($permissionNames);
        $roleMatrix = [
            'Direction' => ['dashboard.view','stock.view','stock.manage','products.manage','warehouses.manage','customers.view','customers.manage','orders.manage','documents.view','documents.manage','payments.view','payments.manage','reports.view','kits.manage','users.manage','purchases.view','purchases.manage'],
            'Coordinateur Projet' => ['dashboard.view','customers.view','customers.manage','orders.manage','documents.view','documents.manage','payments.view','payments.manage','reports.view','kits.manage','purchases.view','purchases.manage'],
            'Agent Logistique' => ['dashboard.view','stock.view','stock.manage','products.manage','warehouses.manage','customers.view','orders.manage','documents.view','documents.manage','payments.view','payments.manage'],
            'Agent Santé' => ['dashboard.view','customers.view','kits.manage','documents.view'],
        ];
        foreach ($roleMatrix as $roleName => $perms) {
            $role = Role::firstOrCreate(['name'=>$roleName,'guard_name'=>'web']);
            $role->syncPermissions($perms);
        }
        $direction->syncRoles([$admin]);
    }
}
