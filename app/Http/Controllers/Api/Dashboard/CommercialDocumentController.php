...
class CommercialDocumentController extends Controller
{
    ...
    {
        $q = CommercialDocument::with(['customer', 'lines.product', 'order', 'source', 'payments.customer'])->latest();
    ...
    {
        return response()->json($document->load(['customer', 'lines.product', 'source', 'creator', 'order', 'payments.customer', 'payments.recorder']));
    }
    ...
    private function createDocument(array $v, int $userId): CommercialDocument
    {
        ...
        // Le stock est décrémenté au moment de la livraison, jamais à la facturation.
    ...
    {
        $doc->loadMissing(['lines','order']);
        $warehouseId = $doc->order?->warehouse_id;
        foreach ($doc->lines as $line) {
            $p = InventoryProduct::lockForUpdate()->findOrFail($line->inventory_product_id);
            $before = (int)$p->stock_quantity; $qty=(int)$line->quantity;
            if (!$restock && $before < $qty) abort(422, 'Stock insuffisant pour '.$p->name);
            $after=$restock?$before+$qty:$before-$qty; $p->update(['stock_quantity'=>$after]);
            if($warehouseId){$ws=\App\Models\WarehouseStock::lockForUpdate()->firstOrCreate(['warehouse_id'=>$warehouseId,'inventory_product_id'=>$p->id],['quantity'=>0,'reserved_quantity'=>0]);if(!$restock && $ws->quantity<$qty)abort(422,'Stock insuffisant dans l’entrepôt pour '.$p->name);$ws->update(['quantity'=>$restock?$ws->quantity+$qty:$ws->quantity-$qty]);}
            $p->movements()->create(['warehouse_id'=>$warehouseId,'type'=>$restock?'entry':'exit','quantity'=>$qty,'stock_before'=>$before,'stock_after'=>$after,'unit_price'=>$line->unit_price,'reference'=>$doc->document_number,'reason'=>$restock?'Avoir / retour client':'Facture / livraison client','user_id'=>$doc->created_by]);
        }
    }
    ...
    public function setStatus(Request $r, CommercialDocument $document)
    {
        $status = $r->validate(['status' => 'required|in:draft,sent,accepted,rejected,cancelled'])['status'];
        if (in_array($document->status, ['closed','cancelled'], true)) abort(422, 'Ce document ne peut plus changer de statut.');
        if ($document->type === 'invoice' && in_array($status, ['draft','sent','rejected'], true) && $document->paid_amount > 0) abort(422, 'Une facture ayant reçu un paiement ne peut plus revenir à ce statut.');
        if ($document->type === 'invoice' && $status === 'cancelled' && $document->paid_amount > 0) abort(422, 'Une facture partiellement ou totalement payée ne peut pas être annulée.');
        $allowed = $document->type === 'proforma'
            ? ['draft'=>['draft','sent','cancelled'],'sent'=>['sent','accepted','rejected','cancelled'],'accepted'=>['accepted','cancelled'],'rejected'=>['rejected'],'cancelled'=>['cancelled']]
            : ['draft'=>['draft','sent','accepted','cancelled'],'sent'=>['sent','accepted','rejected','cancelled'],'accepted'=>['accepted','sent','cancelled'],'partially_paid'=>['partially_paid'],'paid'=>['paid'],'closed'=>['closed'],'cancelled'=>['cancelled']];
        abort_unless(in_array($status, $allowed[$document->status] ?? [], true), 422, 'Transition de statut non autorisée.');
        $document->update(['status'=>$status]);
        return response()->json($document->fresh(['customer','lines.product','source']));
    }
    ...
    public function convert(Request $r, CommercialDocument $document)
    {
        ...
        if ($document->type !== 'proforma' || $document->status !== 'accepted') {
            abort(422, 'Seule une facture proforma acceptée peut devenir une facture.');
        }
    ...
    public function print(CommercialDocument $document, \App\Services\SimplePdfService $pdf)
    {
    ...
    public function pdf(CommercialDocument $document, \App\Services\SimplePdfService $pdf)
    {
        return response($pdf->commercialDocument($document),200,['Content-Type'=>'application/pdf','Content-Disposition'=>'inline; filename="'.$document->document_number.'.pdf"','Cache-Control'=>'no-store']);
    }
